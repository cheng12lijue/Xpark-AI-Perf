...
AI inference performance engineering platform — purpose-built for XFusion FusionXpark hardware, delivering deployment, tuning, and operation of LLM workloads at scale.
...
```bash
cargo install xpark-ai-perf
sudo ~/.cargo/bin/xpark-ai-perf service install
systemctl status xpark-ai-perf
```
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ArcSeekerX/xpark-ai-perf/main/deploy/docker/docker-compose.yml
curl -fsSL https://raw.githubusercontent.com/ArcSeekerX/xpark-ai-perf/main/deploy/docker/.env.docker.example -o .env
...
```bash
git clone https://github.com/ArcSeekerX/Xpark-AI-Perf.git
cd xpark-ai-perf
...
| `DEPLOY_DIR`       | Project path on the remote host, relative to remote home (default`xpark-ai-perf`) |
...
`cargo install` pulls the crate from [crates.io](https://crates.io/crates/xpark-ai-perf)
and compiles it locally. `service install` copies the binary to
`/usr/local/bin`, creates a locked-down `xpark-ai-perf` system user (added
...
> subsequent `sudo xpark-ai-perf …` calls (e.g. `service status`,
...
```bash
sudo systemctl {start|stop|restart} xpark-ai-perf
journalctl -u xpark-ai-perf -f          # follow logs
sudo xpark-ai-perf service status       # same as `systemctl status`
```

Optional overrides live in `/etc/xpark-ai-perf/config.env` — set
`XPARK_AI_PERF_PORT`, `XPARK_AI_PERF_BIND`, `XPARK_AI_PERF_POLL_INTERVAL`,
`XPARK_AI_PERF_GPU_INDEX`, `XPARK_AI_PERF_PROVIDER_API_KEY`, or `RUST_LOG`, then
`sudo systemctl restart xpark-ai-perf`.

### Upgrade

```bash
# Option A
cargo install --force xpark-ai-perf && sudo ~/.cargo/bin/xpark-ai-perf service install

# Option B
cd xpark-ai-perf && git pull && ./deploy/host/install.sh
```

Re-running `service install` is idempotent: it stops the service, swaps the
binary, and starts it again, preserving `/etc/xpark-ai-perf/config.env`.

### Uninstall

```bash
sudo xpark-ai-perf service uninstall         # keep /etc/xpark-ai-perf
sudo xpark-ai-perf service uninstall --purge # remove everything
```

### CLI options

```
xpark-ai-perf [OPTIONS]                 run the server (default)
xpark-ai-perf service install [--prefix /usr/local]
xpark-ai-perf service uninstall [--purge]
xpark-ai-perf service status

  -p, --port <PORT>           Listen port [default: 3000] [env: XPARK_AI_PERF_PORT]
  -b, --bind <BIND>           Bind address [default: 0.0.0.0] [env: XPARK_AI_PERF_BIND]
      --poll-interval <MS>    Polling interval ms [default: 1000] [env: XPARK_AI_PERF_POLL_INTERVAL]
      --gpu-index <IDX>       Optional NVML GPU index to monitor [env: XPARK_AI_PERF_GPU_INDEX]
      --simulate-gpus <N>     Append N fictive GPUs with simulated data (dev aid) [env: XPARK_AI_PERF_SIMULATE_GPUS]
      ...
      --provider-api-key <KEY> Fallback API key for any endpoint [env: XPARK_AI_PERF_PROVIDER_API_KEY]
```

On multi-GPU hosts, Xpark AI Perf monitors all available NVIDIA GPUs by
...
`XPARK_AI_PERF_PROVIDER_API_KEY` as a global fallback covering auto-detected
...
ssh "${DEPLOY_USER}@${DEPLOY_HOST}" tail -f /tmp/xpark-ai-perf.log
...
│       ├── systemd/            xpark-ai-perf.service unit
│       └── config.env.example  /etc/xpark-ai-perf/config.env template
├── dev/
