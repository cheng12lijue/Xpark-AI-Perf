...
docker_args: -v /host/model:/models/model  # Extra docker args passed via VLLM_SPARK_EXTRA_DOCKER_ARGS
...
Use `docker_args` when the model is already present on the host and must be bind-mounted into the container, for example offline ModelScope or local checkpoint directories.
