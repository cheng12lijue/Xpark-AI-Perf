...
RECIPE YAML SCHEMA:
    ...
    docker_args: str       # Optional: Extra docker args passed via VLLM_SPARK_EXTRA_DOCKER_ARGS
...
def load_recipe(recipe_path: Path) -> dict[str, Any]:
    ...
    RECIPE SCHEMA:
        ...
        docker_args (str, optional): Extra docker args passed to launch-cluster.sh
            via VLLM_SPARK_EXTRA_DOCKER_ARGS (e.g., '-v /host/model:/models/model')
    ...
    if not recipe_path.exists():
        ...
        # then search flat recipes/ directory, then recursive subdirectories
        candidates = [
        ...
        # Also search recursively in subdirectories (1x-xpark-solo/, 2x-xpark-cluster/, etc.)
        if not any(c.exists() for c in candidates):
            for subdir in RECIPES_DIR.iterdir():
                if subdir.is_dir():
                    sub_candidates = [
                        subdir / recipe_path.name,
                        subdir / f"{recipe_path.name}.yaml",
                        subdir / f"{recipe_path.name}.yml",
                        subdir / f"{recipe_path.stem}.yaml",
                    ]
        ...
        if not recipe_path.exists():
            ...
            print(f"Searched in: {recipe_path}, {RECIPES_DIR} and subdirectories")
            sys.exit(1)

    # 在它上面加上这段防御代码：
    if str(recipe_path).endswith('.sh'):
        print(f"FATAL ERROR: Attempted to load a Shell script as a YAML recipe!")
        print(f"  File: {recipe_path}")
        print(f"  Please check the logic in main() that generates this temp script.")
        sys.exit(1)
    ...
    recipe.setdefault("docker_args", None)
...
def list_recipes() -> None:
    ...
    recipes = sorted(RECIPES_DIR.glob("*.yaml")) + sorted(RECIPES_DIR.glob("*/**.yaml"))
    if not recipes:
...
def resolve_recipe_docker_args(
    recipe: dict[str, Any], overrides: dict[str, Any]
) -> str | None:
    """
    Resolve optional recipe-level docker args with the same placeholder rules
    used by the command template.
    """
    docker_args = recipe.get("docker_args")
    if not docker_args:
        return None

    if isinstance(docker_args, list):
        docker_args = " ".join(str(part) for part in docker_args)
    elif not isinstance(docker_args, str):
        print("Error: Recipe field 'docker_args' must be a string or list of strings")
        sys.exit(1)

    params = {**recipe.get("defaults", {}), **overrides}
    try:
        return docker_args.format(**params)
    except KeyError as e:
        print(f"Error: Missing parameter in recipe docker_args: {e}")
        print(f"Available parameters: {list(params.keys())}")
        sys.exit(1)
...
Examples:
  ...
  %(prog)s --show-env
    ...
    recipe_docker_args = resolve_recipe_docker_args(recipe, overrides)
    ...
    if args.dry_run:
        ...
        if recipe_docker_args:
            cmd_parts = [
                f"   VLLM_SPARK_EXTRA_DOCKER_ARGS={shlex.quote(recipe_docker_args)}",
                "\\",
                *cmd_parts,
            ]
    ...
    try:
        ...
        if is_cluster:
            ...
            print("Mode: Solo")
        if recipe_docker_args:
            print(f"Extra Docker Args: {recipe_docker_args}")
        print()

        # Execute
        launch_env = os.environ.copy()
        if recipe_docker_args:
            launch_env["VLLM_SPARK_EXTRA_DOCKER_ARGS"] = recipe_docker_args
