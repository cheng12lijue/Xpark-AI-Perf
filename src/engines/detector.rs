...
pub async fn detect_docker_engines() -> Vec<DetectedEngine> {
    ...
    let docker = match Docker::connect_with_local_defaults() {
        ...
        Err(e) => {
            ...
            // Common cause: xpark-ai-perf user not in `docker` group, or the
            // service was started before that group change took effect
            // (`sudo systemctl restart xpark-ai-perf` usually fixes it).
            tracing::info!(
                "Docker detection unavailable (is xpark-ai-perf in the `docker` group \
