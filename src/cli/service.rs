...
//!
//! Exposes `xpark-ai-perf service {install,uninstall,status}` so both install
...
pub enum ServiceCommand {
    ...
    Uninstall {
        /// Also remove /etc/xpark-ai-perf/
        #[arg(long)]
        purge: bool,
    },
    /// Show `systemctl status xpark-ai-perf`.
    Status,
}
...
mod linux {
    ...
    const SERVICE_NAME: &str = "xpark-ai-perf";
    const SERVICE_USER: &str = "xpark-ai-perf";
    const UNIT_PATH: &str = "/etc/systemd/system/xpark-ai-perf.service";
    const CONFIG_DIR: &str = "/etc/xpark-ai-perf";
    const CONFIG_PATH: &str = "/etc/xpark-ai-perf/config.env";
    const CONFIG_EXAMPLE_PATH: &str = "/etc/xpark-ai-perf/config.env.example";

    const UNIT_FILE: &str = include_str!("../../deploy/host/systemd/xpark-ai-perf.service");
    ...
    fn install(prefix: &str) -> Result<()> {
        ...
        println!();
        println!("xpark-ai-perf is installed.");
    ...
    fn uninstall(purge: bool) -> Result<()> {
        ...
        println!();
        println!("xpark-ai-perf is uninstalled.");
        Ok(())
    }
