...
/// Xpark AI Perf — Real-time hardware and LLM monitoring for Linux hosts with NVIDIA GPUs.
#[derive(Parser, Debug)]
#[command(name = "xpark-ai-perf", version, about)]
struct Cli {
    #[command(flatten)]
    run: RunArgs,
...
struct RunArgs {
    ...
    #[arg(
        short = 'b',
        long,
        env = "XPARK_AI_PERF_BIND",
        default_value = "0.0.0.0"
    )]
    bind: String,

    /// Metrics polling interval in milliseconds
    #[arg(long, env = "XPARK_AI_PERF_POLL_INTERVAL", default_value_t = 1000)]
    poll_interval: u64,
    ...
    #[arg(long, env = "XPARK_AI_PERF_GPU_INDEX")]
    ...
    #[arg(long, env = "XPARK_AI_PERF_SIMULATE_GPUS", default_value_t = 0)]
    simulate_gpus: u32,
    ...
    #[arg(long, env = "XPARK_AI_PERF_PROVIDER_API_KEY")]
...
async fn run_server_inner(args: RunArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Xpark AI Perf running at http://{}", addr);
