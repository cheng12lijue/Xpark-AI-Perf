...
: "${DEPLOY_DIR:=${SPARK_DIR:-xpark-ai-perf}}"
...
cleanup() {
    ...
    ssh "${REMOTE}" "pkill -f '[t]arget/release/xpark-ai-perf' || true" 2>/dev/null || true
...
rebuild_backend() {
    ...
    ssh "${REMOTE}" "${REMOTE_ENV} pkill -f '[t]arget/release/xpark-ai-perf' || true"
    if ssh "${REMOTE}" "${REMOTE_ENV} cd ${DEPLOY_DIR} && cargo build --release"; then
        ...
        # XPARK_AI_PERF_PROVIDER_API_KEY unlocks auth-gated /v1/models on
        # auto-detected engines (the binary reads it via clap `env=`).
        # printf %q escapes the value for safe re-parsing by the remote shell.
        backend_env=""
        if [ -n "${XPARK_AI_PERF_PROVIDER_API_KEY:-}" ]; then
            backend_env="XPARK_AI_PERF_PROVIDER_API_KEY=$(printf %q "${XPARK_AI_PERF_PROVIDER_API_KEY}") "
            echo "==> Forwarding XPARK_AI_PERF_PROVIDER_API_KEY to backend"
        fi
        # XPARK_AI_PERF_SIMULATE_GPUS appends fictive GPUs for multi-GPU UI
        # testing on the single-GPU Spark (the binary reads it via clap `env=`).
        if [ -n "${XPARK_AI_PERF_SIMULATE_GPUS:-}" ]; then
            backend_env="${backend_env}XPARK_AI_PERF_SIMULATE_GPUS=$(printf %q "${XPARK_AI_PERF_SIMULATE_GPUS}") "
            echo "==> Forwarding XPARK_AI_PERF_SIMULATE_GPUS to backend"
        fi
        ssh "${REMOTE}" "cd ${DEPLOY_DIR} && > /tmp/xpark-ai-perf.log && (nohup env ${backend_env}./target/release/xpark-ai-perf >> /tmp/xpark-ai-perf.log 2>&1 < /dev/null &) &"
...
ssh "${REMOTE}" "tail -n0 -f /tmp/xpark-ai-perf.log" 2>/dev/null &
PIDS+=($!)
