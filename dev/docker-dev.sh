...
Modes (exactly one required):
  ...
  --deploy-ghcr         Build a multi-arch image (linux/arm64,linux/amd64) and
                        ...
                        XPARK_AI_PERF_IMAGE=${GHCR_IMAGE}:${GHCR_TAG} in the
...
Environment (read from repo-root .env, same as dev/dev.sh):
  ...
  DEPLOY_DIR   Project path on the remote host (default 'xpark-ai-perf')
...
# is the port the app binds directly; set XPARK_AI_PERF_PORT in .env to move it
# (e.g. off :3000 when a cargo-installed instance already owns that port).
DASH_PORT="${XPARK_AI_PERF_PORT:-3000}"
...
deploy_remote() {
    ...
    ssh "${REMOTE}" bash -lc "set -e
        ...
        echo
        echo '--- recent logs ---'
        \$compose logs --tail=20 xpark-ai-perf || true
    "
...
deploy_ghcr() {
    ...
    echo "${GHCR_PAT}" | docker login ghcr.io -u "$(gh api user --jq .login 2>/dev/null || echo ArcSeekerX)" --password-stdin
...
remote_logs() {
    needs_remote
    ssh "${REMOTE}" "cd '${DEPLOY_DIR}' && docker compose --env-file .env -f deploy/docker/docker-compose.yml logs -f --tail=100 xpark-ai-perf"
}
