...
port 8000, for validating the xpark-ai-perf multi-engine view.
...
With `xpark-ai-perf` running (via `systemctl start xpark-ai-perf` or
...
1. **`xpark-ai-perf` isn't in the `docker` group yet.** The service
   ...
   ```bash
   sudo usermod -aG docker xpark-ai-perf
   sudo systemctl restart xpark-ai-perf
   journalctl -u xpark-ai-perf -n 50 | grep -i docker
   ```
   ...
   `sudo -u xpark-ai-perf id` and check for `docker`.
