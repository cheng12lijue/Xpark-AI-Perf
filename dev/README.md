...
Development-only scripts for xpark-ai-perf. Configuration is read from a
repo-root `.env` file — copy `dev/.env.example` to `.env` and edit before running.

For **production installs**, use `cargo install xpark-ai-perf` or
...
5. streams remote backend logs from `/tmp/xpark-ai-perf.log`
...
| Variable                           | Purpose                                                                  |
|------------------------------------|--------------------------------------------------------------------------|
| `XPARK_AI_PERF_PROVIDER_API_KEY` | Fallback API key for auth-gated engines. Forwarded to the remote backend. |
| `XPARK_AI_PERF_SIMULATE_GPUS`    | Append N fictive GPUs with simulated data for multi-GPU UI testing (see below). |
...
1. Add the key to your repo-root `.env`:
   ...
   ```bash
   XPARK_AI_PERF_PROVIDER_API_KEY=your-vllm-api-key
   ```

2. Restart `./dev/dev.sh`. On backend start you'll see
   `==> Forwarding XPARK_AI_PERF_PROVIDER_API_KEY to backend`.
3. In the streamed remote log (`/tmp/xpark-ai-perf.log`) and the vLLM
