#!/usr/bin/env bash
#
# Source-build installer for xpark-ai-perf.
#
# Run from the root of a cloned repository on the target Linux host as your
# normal user — do NOT prefix with `sudo`. The script builds the frontend and
# binary as you, and escalates to sudo only when wiring up the systemd service.
#
#   ./deploy/host/install.sh              # build + install + enable service
#   ./deploy/host/install.sh --no-service # build + install binary only
#   ./deploy/host/install.sh --uninstall  # remove service + binary
#
# Primary install path is `cargo install xpark-ai-perf`. Use this script when
# you want to install from a local checkout (air-gapped, auditing, or
# development installs of unreleased code).

set -euo pipefail

# Two levels up: this script lives in deploy/host/.
PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PREFIX="/usr/local"
MODE="install"

usage() {
    cat <<'EOF'
Usage: install.sh [--prefix PATH] [--no-service] [--uninstall] [--purge]

Options:
  --prefix PATH   Install prefix (default: /usr/local)
  --no-service    Build and install the binary only; skip systemd wiring
  --uninstall     Remove the service and binary
    --purge         With --uninstall, also remove /etc/xpark-ai-perf
  -h, --help      Show this help
EOF
}

NO_SERVICE=0
PURGE=0

while [[ $# -gt 0 ]]; do
    case "$1" in
        --prefix)
            PREFIX="${2:?--prefix requires a path}"
            shift 2
            ;;
        --no-service)
            NO_SERVICE=1
            shift
            ;;
        --uninstall)
            MODE="uninstall"
            shift
            ;;
        --purge)
            PURGE=1
            shift
            ;;
        -h|--help)
            usage
            exit 0
            ;;
        *)
            echo "Unknown option: $1" >&2
            usage >&2
            exit 2
            ;;
    esac
done

require() {
    command -v "$1" >/dev/null 2>&1 || {
        echo "error: \`$1\` is required but not installed or not on PATH" >&2
        exit 1
    }
}

refuse_root() {
    if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
        cat >&2 <<'MSG'
error: do not run this script with sudo.

This installer builds the frontend (npm) and binary (cargo) as your regular
user, and will escalate to sudo only when installing the systemd service.
Running under sudo strips PATH and breaks lookups for cargo/npm installed
in your home (nvm, rustup, etc.).

Re-run without sudo:
  ./deploy/host/install.sh                 # install
  ./deploy/host/install.sh --uninstall     # uninstall

You'll be prompted for your sudo password when the service is wired up.
MSG
        exit 1
    fi
}

preflight() {
    if [[ "$(uname -s)" != "Linux" ]]; then
        echo "error: this installer only supports Linux" >&2
        exit 1
    fi
    require systemctl
    require sudo
}

uninstall() {
    refuse_root
    preflight
    local bin="${PREFIX}/bin/xpark-ai-perf"
    if [[ -x "$bin" ]]; then
        local purge_args=()
        if [[ $PURGE -eq 1 ]]; then
            purge_args+=(--purge)
        fi
        # The binary self-escalates to sudo for the uninstall operation.
        "$bin" service uninstall "${purge_args[@]}"
    else
        echo "xpark-ai-perf is not installed at $bin — nothing to do"
    fi
}

install_from_source() {
    refuse_root
    preflight
    require cargo
    require npm
    require node

    echo "==> Building frontend"
    (cd "${PROJECT_ROOT}/frontend" && npm ci && npm run build)

    echo "==> Building release binary"
    (cd "$PROJECT_ROOT" && cargo build --release --locked)

    local built="${PROJECT_ROOT}/target/release/xpark-ai-perf"
    if [[ ! -x "$built" ]]; then
        echo "error: expected binary at $built but it is missing" >&2
        exit 1
    fi

    if [[ $NO_SERVICE -eq 1 ]]; then
        echo "==> Installing binary to ${PREFIX}/bin/xpark-ai-perf (no service)"
        sudo install -m 0755 "$built" "${PREFIX}/bin/xpark-ai-perf"
        echo "done"
        return
    fi

    echo "==> Installing service (sudo)"
    sudo "$built" service install --prefix "$PREFIX"
}

case "$MODE" in
    install)   install_from_source ;;
    uninstall) uninstall ;;
esac
