# Running xpark-ai-perf with Docker
...
to **`ghcr.io/ArcSeekerX/xpark-ai-perf`**, tagged `:vX.Y.Z`, `:vX.Y`, and
`:latest`.
...
docker compose up -d
docker compose logs -f xpark-ai-perf
```

By default Compose **pulls** `ghcr.io/ArcSeekerX/xpark-ai-perf:latest`. To
...
Pin a specific version or a dev tag with `XPARK_AI_PERF_IMAGE` in `.env`:

```bash
XPARK_AI_PERF_IMAGE=ghcr.io/ArcSeekerX/xpark-ai-perf:v0.10.0
```
...
single-tenant GPU box. In this mode `XPARK_AI_PERF_PORT` is the port the app
...
For `docker run`, the equivalent is `--gpus all`. Xpark AI Perf monitors all
visible NVIDIA GPUs by default; set `XPARK_AI_PERF_GPU_INDEX` to focus on one
specific device.
...
| `XPARK_AI_PERF_IMAGE`             | `…:latest`  | Image Compose runs when not building from source.    |
| `XPARK_AI_PERF_PORT`              | `3000`      | Listen port.                                         |
| `XPARK_AI_PERF_BIND`              | `0.0.0.0`   | Bind address.                                        |
| `XPARK_AI_PERF_POLL_INTERVAL`     | `1000`      | Metrics polling interval (ms).                       |
| `XPARK_AI_PERF_GPU_INDEX`         | _(unset)_   | Optional NVML GPU index; unset monitors all GPUs.    |
| `XPARK_AI_PERF_SIMULATE_GPUS`     | `0`         | Dev aid: append N fictive GPUs with simulated data.  |
| `XPARK_AI_PERF_PROVIDER_API_KEY`  | _(unset)_   | Fallback API key for auth-gated engines.             |
...
```bash
docker inspect xpark-ai-perf --format '{{.State.Health.Status}}'   # -> healthy
```
...
Or pin to a new explicit `XPARK_AI_PERF_IMAGE` tag and re-run `up -d`.
