...
- **Key Management**: credential management tab supporting API Key add, edit, delete, reveal and copy.
- **vLLM recipes**: full recipe catalog organized into `1x-xpark-solo`, `2x-xpark-cluster`, `3x-xpark-cluster`, `4x-xpark-cluster` directories.

### Changed

- Default landing page set to Model Registry tab.
- `run-recipe.py` updated to search recipes recursively in subdirectories.
- Recipe card display simplified to show filename without directory prefix.
...
- Frontend Docker build failure: copy CHANGELOG.md into build stage.
...
- Version indicator moved from footer to header as a subscript badge.
- Docker image references lowercased to `ghcr.io/arcseekerx/...`.
- `docker run` quick-start switched to `--network host`.
...
[0.4.0]: https://github.com/ArcSeekerX/Xpark-AI-Perf/releases/tag/v0.4.0
